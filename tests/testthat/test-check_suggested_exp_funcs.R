...
test_that("check_suggested_exp_funcs returns no exported functions message", {
  ...
  #this will need rlang 1.1.5 to be installed before installing haven
  #see https://stackoverflow.com/questions/61814754/problems-with-installing-package-haven

  # Install rlang 1.1.5 before installing haven
  # Install rlang if it can be removed
  if (!"rlang" %in% installed.packages()[, "Package"]) {
    devtools::install_version("rlang", version = version, repos = "http://cran.us.r-project.org")
    message("Successfully installed rlang version ", version)
  } else {
    message("rlang is already installed and could not be removed.")
  }

  dp <- system.file("test-data", "haven-2.5.4.tar.gz",
                    package = "risk.assessr")
  ...
  if (package_installed == TRUE ) {
    ...
    package <- c("cli", "forcats", "hms", "lifecycle",
                 "methods", "readr", "rlang",
                 "tibble", "tidyselect", "vctrs",
                 "covr", "crayon", "fs", "knitr",
                 "pillar", "rmarkdown", "testthat", "utf8",
                 "cpp11")
    ...
    expect_true(all(!is.na(result$source)))
    expect_true(all(!is.na(result$suggested_function)))
    expect_true(all(!is.na(result$targeted_package)))
    expect_true(all(!is.na(result$message)))
  }
})
